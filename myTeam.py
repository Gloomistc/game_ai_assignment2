...
class MCTSNode(object):
    ...
    def node_expanded(self):
        ...
        if self.unexploredActions != []:
            ...
            child_node = MCTSNode(
                next_game_state, self.agent, action, self, self.enemy_pos, self.borderline)
    ...
    def get_feature(self):
        ...
        feature['distance'] = min([self.agent.getMazeDistance(
            current_pos, borderPos) for borderPos in self.borderline])
        return feature
...
class OffensiveAgent(CaptureAgent):
    ...
    def detect_enemy_approaching(self, gameState):
        ...
        for g in ghosts:
            distance = self.getMazeDistance(
                myPos, gameState.getAgentPosition(g))
            if distance <= 5:
    ...
    def chooseAction(self, gameState):
        ...
        if isPacman:
            print("1")
            appr_ghost_pos = [gameState.getAgentPosition(
                g) for g in self.detect_enemy_approaching(gameState)]
    ...
    def get_off_features(self, gameState, action):
        ...
        if next_tate.getAgentState(self.index).numCarrying > gameState.getAgentState(self.index).numCarrying:
            ...
            if len(self.getFood(next_tate).asList()) > 0:
                features['minDistToFood'] = self.get_min_dist_to_food(
                    next_tate)
        return features

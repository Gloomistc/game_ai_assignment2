...
class ReflexAgent(Agent):
    ...
    def getAction(self, gameState):  # 通过评估获得下一步的action， 返回下一步的location
        ...
        # get all the action's score
        scores = [self.evaluationFunction(
            gameState, action) for action in legalMoves]
        bestScore = max(scores)  # get best score
        bestIndices = [index for index in range(
            len(scores)) if scores[index] == bestScore]  # get best action
    ...
    def bfs_findFood(self, pacman_x, pacman_y, newFood):
        Q = queue.Queue()
        walked, dx, dy, Width, Height = [], [1, 0, -1, 0], [0, 1,
                                                            0, -1], newFood.width, newFood.height  # four direction
        Q.put([pacman_x, pacman_y, 0])  # start point
        # record that the start point has reached
        walked.append((pacman_x, pacman_y))
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        successorGameState = currentGameState.generatePacmanSuccessor(
            action)  # Get current Pacman location
        ...
        closest_ghost = min([util.manhattanDistance(newPos, ghost)
        ...
        if closest_ghost >= ghost_dis:
            ...
            # Bfs:
            closest_food = self.bfs_findFood(
                pacman_x, pacman_y, newFood)  # !!!
            if closest_food == -1:
        ...
        if not agentIndex:  # Pacman
            ...
            for action in pacmanAction:
                successor_state = gameState.generateSuccessor(
                    agentIndex, action)  # generate new state
                res = self.minimaxFunc(
                    1, successor_state, now_depth + 1)  # recurse
            ...
            for action in ghostAction:  # Recurse
                ...
                if agentIndex >= gameState.getNumAgents() - 1:  # nowAgent is the final ghost
                    MIN = min(MIN, self.minimaxFunc(
                        0, successor_state, now_depth + 1))  # renew MIN value
                else:  # still ghost
                    MIN = min(MIN, self.minimaxFunc(
                        agentIndex + 1, successor_state, now_depth))
            return MIN
    ...
    def getAction(self, gameState):
        ...
        for action in gameState.getLegalPacmanActions():
            if action == "Stop":
                continue
            successor_state = gameState.generateSuccessor(0, action)
            res = self.AlphaBeta_minimaxFunc(
                0, successor_state, 0, alpha, beta)
            if res > max_value:
                max_value = res
                best_expect_action.clear()
                best_expect_action.append(action)
        ...
        if agentIndex == 0:  # Pacman
            ...
            for action in pacmanAction:
                successor_state = gameState.generateSuccessor(
                    agentIndex, action)  # generate new state
                res = self.AlphaBeta_minimaxFunc(
                    1, successor_state, now_depth + 1, alpha, beta)  # recurse
                if res is not None:
                    MAX = max(MAX, res)  # renew MAX value
                if MAX > beta:  # cut
                    return MAX
            ...
            for action in ghostAction:  # Recurse
                ...
                res = self.AlphaBeta_minimaxFunc(
                    (agentIndex+1) % gameState.getNumAgents(), successor_state, now_depth, alpha, beta)
                if res is not None:
                    MIN = min(MIN, res)
                if MIN < alpha:
                    return MIN
...
def betterEvaluationFunction(currentGameState):
    ...
    for capsule in CapsulesPos:
        CapsuleScore = max(CapsuleScore, 1.5 /
                           float(util.manhattanDistance(capsule, currentPos)))
